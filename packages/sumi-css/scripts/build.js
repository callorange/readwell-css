...
const BANNER = `/*!
 * Sumi CSS v${pkg.version}
 * https://github.com/callorange/readwell-css/tree/main/packages/sumi-css
...
function safeMinifyCSS(css) {
  ...
  // Note: url() can contain quoted strings with parens (e.g., SVG transforms like rotate(...))
  const tokenRegex = /(url\(\s*(?:"[^"\\]*(?:\\.[^"\\]*)*"|'[^'\\]*(?:\\.[^'\\]*)*'|[^)]*)\)|"[^"\\]*(?:\\.[^"\\]*)*"|'[^'\\]*(?:\\.[^'\\]*)*')/g;
  let masked = css.replace(tokenRegex, (match) => {
    const key = `___SUMI_TOKEN_${tokens.length}___`;
    tokens.push(match);
    return key;
  });
  ...
  for (let i = 0; i < tokens.length; i++) {
    masked = masked.replace(`___SUMI_TOKEN_${i}___`, () => tokens[i]);
  }
...
export function build() {
  ...
  // 1. Asset synchronization: copy assets/ -> dist/assets/
  let assetCount = 0;
  if (fs.existsSync(assetsDir)) {
    ensureDir(distAssetsDir);
    fs.cpSync(assetsDir, distAssetsDir, { recursive: true });
    assetCount = fs.readdirSync(distAssetsDir).length;
  }
  ...
  const minifiedCSS = BANNER + safeMinifyCSS(bundledCSS) + '\n/*# sourceMappingURL=sumi.min.css.map */\n';

  const fullPath = path.join(distDir, 'sumi.css');
  const fullMapPath = path.join(distDir, 'sumi.css.map');
  const minPath = path.join(distDir, 'sumi.min.css');
  const minMapPath = path.join(distDir, 'sumi.min.css.map');

  fs.writeFileSync(fullPath, fullCSS, 'utf-8');
  fs.writeFileSync(minPath, minifiedCSS, 'utf-8');

  const fullMap = generateSourceMap('sumi.css', fileList);
  const minMap = generateSourceMap('sumi.min.css', fileList);
  ...
  console.log(`  - dist/sumi.css (${fullSize} KB) + sumi.css.map`);
  console.log(`  - dist/sumi.min.css (${minSize} KB) + sumi.min.css.map`);
  console.log(`  - dist/assets/ (${assetCount} files copied)`);

  return { fullPath, minPath, fullMapPath, minMapPath, distAssetsDir, fullSize, minSize, assetCount };
}
